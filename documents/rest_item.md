...
| /api/addpublish | Publish를 추가한다. status는 usethis, working, notuse 로 설정할 수 있다. | project, name, task, key, path, status, (mainversion), (subversion), (subject), (filetype), (kindofusd) | `$ curl -X POST -d "project=TEMP&name=SS_0010&task=comp&key=pub&path=/path/file.nk&mainversion=1&subversion=1&subject=roto&filetype=.usd&kindofusd=component&status=usethis" https://csi.lazypic.org/api/publish`|
