...
| /api/publish | Publish를 셋팅한다. | project, task, key, path, version | `$ curl -d "project=TEMP&task=comp&key=pub&path=/path/file.nk&version=1" https://csi.lazypic.org/api/publish`|
