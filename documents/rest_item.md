...
| /api3/items | 아이템을 검색하고 가지고 오기 | project, searchword, status상태 | `$ curl "https://csi.lazypic.org/api3/items?project=TEMP&searchword=SS&wip=true"` 또는 `$ curl "https://csi.lazypic.org/api3/items?project=TEMP&searchword=task:mm+user:jason&wip=true"` |
