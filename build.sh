...
GOOS=linux GOARCH=amd64 go build -ldflags "-X main.SHA1VER=`git rev-parse HEAD` -X main.DNS=csi.idea.co.kr -X main.BUILDTIME=`date -u +%Y-%m-%dT%H:%M:%S` -X main.COMPANY=Digitalidea -X main.MAILDNS=idea.co.kr -X main.DBIP=10.0.90.249" -o ./bin/linux_di/${APP} *.go
